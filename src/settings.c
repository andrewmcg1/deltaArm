...
int settings_load_from_file(const char* path)
{
    ...
    PARSE_DOUBLE_MIN_MAX(base_side_length, 0.0, 150.0)
#ifdef DEBUG
    fprintf(stderr, "base_side_length: %d\n", settings.base_side_length);
#endif

    PARSE_DOUBLE_MIN_MAX(end_effector_side_length, 0.0, 50.0)
#ifndef DEBUG
    fprintf(stderr, "end_effector_side_length: %d\n", settings.end_effector_side_length);
#endif

    PARSE_DOUBLE_MIN_MAX(top_link_length, 0.0, 150.0)
#ifdef DEBUG
    fprintf(stderr, "top_link_length: %d\n", settings.top_link_length);
#endif

    PARSE_DOUBLE_MIN_MAX(bottom_link_length, 0.0, 150.0)
#ifdef DEBUG
    fprintf(stderr, "bottom_link_length: %d\n", settings.bottom_link_length);
#endif

    PARSE_STRING(flight_controller_uart)
#ifdef DEBUG
    fprintf(stderr, "flight_controller_uart: %s\n", settings.flight_controller_uart);
#endif

    // GYROSCOPE LOW PASS FILTER TIME CONSTANT MULTIPLIER
    PARSE_DOUBLE_MIN_MAX(moc_lpf_timeConst_multiplier, 0, 1000)
